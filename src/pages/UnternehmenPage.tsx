        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('laender')}>
                <span className="inline-flex items-center gap-1">
                  Tätigkeitsländer
                  {sortKey === 'laender' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('epr_registrierungsnummern')}>
                <span className="inline-flex items-center gap-1">
                  EPR-Registrierungsnummern (je Land)
                  {sortKey === 'epr_registrierungsnummern' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('verantwortlich_vorname')}>
                <span className="inline-flex items-center gap-1">
                  Vorname verantwortliche Person
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('verantwortlich_funktion')}>
                <span className="inline-flex items-center gap-1">
                  Funktion / Position
                  {sortKey === 'verantwortlich_funktion' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell>{record.fields.laender ?? '—'}</TableCell>
                ...
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.epr_registrierungsnummern ?? '—'}</span></TableCell>
                <TableCell>{record.fields.verantwortlich_vorname ?? '—'}</TableCell>
                <TableCell>{record.fields.verantwortlich_nachname ?? '—'}</TableCell>
                <TableCell>{record.fields.verantwortlich_funktion ?? '—'}</TableCell>
