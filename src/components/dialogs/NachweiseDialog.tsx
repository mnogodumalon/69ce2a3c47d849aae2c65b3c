...
export function NachweiseDialog({ open, onClose, onSubmit, defaultValues, verpackungstypenList, enablePhotoScan = true, enablePhotoLocation = true }: NachweiseDialogProps) {
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="verpackungstyp_ref" entity="Verpackungstypen">\n${JSON.stringify(verpackungstypenList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["verpackungstyp_ref"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const verpackungstyp_refName = raw['verpackungstyp_ref'] as string | null;
        if (verpackungstyp_refName) {
          const verpackungstyp_refMatch = verpackungstypenList.find(r => matchName(verpackungstyp_refName!, [String(r.fields.verpackungs_id ?? '')]));
          if (verpackungstyp_refMatch) merged['verpackungstyp_ref'] = createRecordUrl(APP_IDS.VERPACKUNGSTYPEN, verpackungstyp_refMatch.record_id);
        }
