...
export function RegelstatusDialog({ open, onClose, onSubmit, defaultValues, verpackungstypenList, enablePhotoScan = true, enablePhotoLocation = true }: RegelstatusDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'regelstatus');
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <Select
              value={extractRecordId(fields.verpackungstyp_status_ref) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, verpackungstyp_status_ref: v === 'none' ? undefined : createRecordUrl(APP_IDS.VERPACKUNGSTYPEN, v) }))}
            >
              <SelectTrigger id="verpackungstyp_status_ref"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            ...
            <Select
              value={lookupKey(fields.konformitaetsstatus) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, konformitaetsstatus: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="konformitaetsstatus"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
